<script lang="ts">
	...
	function search(e: any) {
		...
		if (value) {
			const filteredPatients = nurses.filter((patient) => {
				return (
	...
	let showAddNurseModal: boolean = false;
</script>

<Modal bind:showModal={showAddNurseModal} action="?/add" title="Add New Nurse">
...
<header class="py-5 px-8 border-b border-gray-100 flex justify-between">
	...
	<nav>
		<button
			...
			on:click={() => (showAddNurseModal = true)}
		>
			New
		</button>
	</nav>
	...
	<div class="relative overflow-x-auto p-8">
		...
		<table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
			<thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
				<tr>
					<th scope="col" class="px-6 py-3"> Nurse </th>
					...
					>
						<th scope="row" class="">
							...
							>
								...
								<div class="pl-3">
									<div class="text-base font-semibold">{nurse.firstname} {nurse.lastname}</div>
									<div class="font-normal text-gray-500">{nurse.email}</div>
								</div>
							</a>
						</th>
						<td class="px-6 py-4 space-y-1 capitalize"> {nurse.gender} </td>
						<td class="px-6 py-4"> {nurse.age} </td>
						<td class="px-6 py-4">
							<form action={`/admin/nurses/${nurse.id}/?/delete`} method="post">
								<button
									type="submit"

<script lang="ts">
    ...
    function search(e: any) {
        ...
        if (value) {
            doctors = data.doctors.filter((doctor) => {
                return doctor.speciality?.toLowerCase() == value?.toLowerCase()
            });
        } else {
...
<section class="bg-white p-8">
    <a href={`/nurse/patients/${data.userId}`}
       class="bg-black px-6 py-2 rounded-md font-bold text-white mb-8 block w-20">Back</a>

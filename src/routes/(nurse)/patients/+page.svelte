			...
			<tbody>
				{#each data.patients as patient, index}
					...
					>
						...
						>
                        <a href="/patients/{patient.id}" class="hover:text-blue-600">{patient.firstname} {patient.lastname}</a>
						
						</th>
						<td class="px-6 py-4"> {patient.email} </td>
						<td class="px-6 py-4 capitalize"> {patient.age} </td>
						<td class="px-6 py-4 capitalize"> {patient.gender} </td>

...
{#if data.patients.length >= 1}
	<div class="">
		<table class="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
			...
			<tbody>
				{#each data.patients as patient, index}
					<tr class="hover:bg-gray-200">
						<td class="py-2 px-4">{index}</td>
						<td class="py-2 px-4 capitalize">{patient.name}</td>
						<td class="py-2 px-4 capitalize">{patient.age}</td>
	...
	{:else}
	<h2>Sorry no patients available</h2>
{/if}
...
<Modal bind:showModal={show}>
	...
	<form action="/nurses?/add" use:enhance method="post" class="w-96">
		<h2 class="text-2xl font-semibold mb-4">Add Patient</h2>

		<div class="mb-4">
			<label class="block text-gray-700 text-sm font-bold mb-2" for="nurseName"> Patient Name </label>
			<input
		...
		<div class="mb-4">
			<label class="block text-gray-700 text-sm font-bold mb-2" for="nurseAge"> Patient Age </label>
			<input
				class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
				id="nurseAge"
				type="number"
